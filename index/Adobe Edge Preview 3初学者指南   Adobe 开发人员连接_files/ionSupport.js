...
var ionSupportBadge = function(){
    ...
    config = {
		...
		proxyFile : serverRoot + 'badge/v4/proxy.v4',	//Source file of the embeded SWF proxy
		...
		tumbsTotalURL : serverRoot + 'api/v1/thumbs/avgthumb.html',     //API URL path for retrieving the thumbs total.
	...
	Core.registerToManager = function() {
		config.managerKey = ionResponse.attachConsumer(this);
	};
	...
	Core.getThumbRatingsTotal = function() {
	
		var parameters = Core.getBasicServiceParams();
		parameters.push(['where', 'resource_id:' + opt.url]);

		var request = new ServerRequest();
		request.url = config.tumbsTotalURL;
		request.method = "POST";
		request.params = parameters;
		request.id = Hasher.grind().toString();
		request.onSuccess = Core.updateSupportThumbRatingsTotal;
		request.onError = function(){
	...
	// Update the total thumbs rating display module.
	Core.updateSupportThumbRatingsTotal = function(responseBody) {
		...
		if(responseBody.response[0].total == 0) {
			Msg.show("NO_RATING");
		} else {
			Msg.remove("NO_RATING");
			up = responseBody.response[1][0].up;
			down = responseBody.response[1][0].down;
			Core.renderTotalsDisplay(up, down);
		}
	    }else {
            Template.ratingsTotalModule.innerHTML  = '<div class="vAligned"><span>Unable to retrieve stats.</span></div>';
	    }
	}


	// Render the ratings total display text.
	Core.renderTotalsDisplay = function (up, down) {

		var total = up + down;
		var percentage = Math.ceil((up*100)/total);
		var message = "";
		if(opt.totalTypeDisplay === 'absolute') {
			message = up + ' out of ' + total + ' visitors found this article helpful.';
		} else if (opt.totalTypeDisplay === 'percentage') {
			message = percentage + '% visitors found this article helpful.';
		} else if (opt.totalTypeDisplay === 'percentage-relative') {
			message = percentage + '% out of ' + total + ' visitors found this helpful.';
		}
		Template.ratingsTotalModule.innerHTML = '<div class="vAligned"><span>' + message + '</span></div>';
	}
	...
	Core.updateSupportRating = function(responseBody){
		...
		//if show ratings total is enabled make sure you update the statistics.
		if(opt.showTotals)
			Core.getThumbRatingsTotal();
	};
	...
	Template.buildCallToAction = function(){
		...
		//Build the total ratings module content
		Template.buildRatingsTotalModule();
	...
	/**
	 * Build the total ratings module container and append to the add form.
	 */
	Template.buildRatingsTotalModule = function() {
		if(opt.showTotals) {
		    Template.buildRatingsTotalModuleContainer();
		    Core.getThumbRatingsTotal();
		}
	};
	...
	//fbocse
	/**
	 * Build the ratings total module container and append to the add form.
	 */
	Template.buildRatingsTotalModuleContainer =  function(){
	
		/*
		// this is a css fix.
		var clearFix = Template.breedElement({ 
			type:'div',
			className: 'ionClearFix', 
			parent: Template.addForm
		});	
		clearFix.spawn();
		*/

		var ratingsContainer  = (opt.votesContainer == null) ? Template.addForm : opt.votesContainer;


		// TODO - please document this change.
		var module = Template.breedElement({
			type: 'div',
			className: 'ionRatingsTotalContainer vAlignHolder',
			parent: ratingsContainer
		});
		...
		//get a DOM reference to the rating module from the page
		Template.ratingsTotalModule = module.reference();
	};
	...
	Template.buildRatingControl = function(){
		...
		/**
		 * Retrieve the active thumb
		 * @param {Object} name
		 * @param {Object} value
		 */
		var _getActiveThumbRadioInput = function(name, value){
			var thumb;
		...
		var _getActiveThumbButton = function(name, value, text, thumbClass, buttonWidth, buttonTitle) {
			...
			try{
                		thumbElement = document.createElement(' <a title="' + buttonTitle + '" ' +
                                                      ' class="ionThumbButton '+thumbClass+'" ' +
                                                      ' name="' + name + '" value="' + value + '" onclick=";return false;">');

				thumbElement.innerHTML = '<span>'+text+'</span>';
			}
			catch(e){
			}
			
			//DOM Compliant Browsers
			if (!thumbElement || !thumbElement.name) {
            
			thumbElement = document.createElement('a');
			thumbElement.title = buttonTitle;
			thumbElement.className = 'ionThumbButton ' + thumbClass;
			thumbElement.name = name;

			thumbText = document.createElement('span');
			thumbText.innerHTML = text;
			thumbElement.appendChild(thumbText);
		
				if(thumbElement.attachEvent) {
					thumbElement.attachEvent('onclick', function() { return false; });
				} else {
					thumbElement.setAttribute('onclick', ';return false;');
				}
			
			thumbElement.value = value;
		...
		var _buildRatingModule = function() {

			if (opt.voteTypeDisplay === 'thumbs') {
				buildThumbButtons();
			} else {
				buildSimpleButtons();
			}
		...
		var _buildRatingModuleContainer = function() {
		...
		var buildThumbsRating = function(){
			return _buildRatingModule();
		}
		
		var _toggleInputClass = function(el, cssClass){
			
            	var elem = el;
//		var elem = el.getElementsByTagName('img')[0];
		
			// if the elements has the CSS class
			if (elem.className.match(new RegExp('(\\s|^)' + cssClass + '(\\s|$)'))) {
		...
		var buildThumbButtons = function() {
			...
			var yesInput = _getActiveThumbButton('ionThumbsRating', 1, 'Yes', 'ion-thumb-up', '50', 'This was helpful');
			var noInput = _getActiveThumbButton('ionThumbsRating', 0, 'No', 'ion-thumb-down', '50', 'This was not helpful');
			
			//make sure that the user has not submited a rating.
			if (User.flag['rating'] === null || User.flag['rating'] == false) {
				//set the rating when clicking the button
				...
				//toogle the thumb images.
				Util.setEventListener(yesInput, 'mouseover', function(){
					_toggleInputClass(yesInput, 'ion-thumb-up-active');
				});
				//toogle the thumb images.
				Util.setEventListener(yesInput, 'mouseout', function(){
					_toggleInputClass(yesInput, 'ion-thumb-up-active');
				});
				Util.setEventListener(noInput, 'mouseover', function() {
					_toggleInputClass(noInput, 'ion-thumb-down-active');
				});
				Util.setEventListener(noInput, 'mouseout', function() {
					_toggleInputClass(noInput, 'ion-thumb-down-active');
				});
            ...
            container.ghost().appendChild(fix);
		 ...
		 */
		var buildSimpleButtons = function(){
			...
			var yesInput = _getActiveThumbRadioInput('ionThumbsRating', 1);
			var noInput = _getActiveThumbRadioInput('ionThumbsRating', 0);
	...
	Template.buildRatingLabel =  function(){
        ...
        labelHolder.className = 'vAlignHolder';
        labelHolder.style.cssFloat = 'left';
        labelHolder.style.styleFloat = 'left';

        var labelWrapper = document.createElement('div');
        labelWrapper.className = 'vAligned';        
        ...
        labelHolder.appendChild(labelWrapper);
		Template.ratingModule.appendChild(labelHolder);
	};
	...
	Template.breedElement = function(configs) {
		...
		var valid = { types:['div', 'form', 'span', 'h1', 'h2', 'h3',  'ul', 'a', 'textarea', 'br'] };
		var Core = {};
		var config = {};
		var init = configs;
		var element;
...
//					(	(typeof(config.parent.appendChild["call"] != 'undefined'))) || 
//						(typeof config.parent.appendChild.call != 'undefined')) 
...
var ionComProxy = {
	...
	return  {
		...
		load: function(){
			...
			var _allowedTotalDisplayFormats = Util.arrayToString(['absolute', 'percentage', 'percentage-relative']);			
			var _allowedVoteTemplates = Util.arrayToString(['simple','thumbs']);
			...
			opt.votesContainer = (typeof this.votesContainer == 'object' ) ? this.votesContainer : null;
			...
			opt.showTotals = (this.showTotals === true) ? true : false;
			opt.totalTypeDisplay = (typeof this.totalTypeDisplay == 'string' && _allowedTotalDisplayFormats.indexOf(this.totalTypeDisplay) > -1) ? this.totalTypeDisplay : 'absolute';
			opt.voteTypeDisplay = (typeof this.voteTypeDisplay == 'string' && _allowedVoteTemplates.indexOf(this.voteTypeDisplay) > -1) ? this.voteTypeDisplay : 'simple';
			...
			this.showTotals = opt.showTotals;
			this.totalTypeDisplay = opt.totalTypeDisplay;
			this.voteTypeDisplay = opt.voteTypeDisplay;
