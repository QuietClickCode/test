...
if (s_adobe.c_r("omniture_optout")!=1 && s_URL.indexOf("/misc/optout")==-1) {
		...
		if (s_URL.indexOf("adobe.com")!=-1) {
			...
			function s_adobe_genesis_dmdbase(a){this.conf=a;
			this.tl=function(){if("done"==this.c_r(this.conf.var_name))return;if(typeof window[this.conf.var_name]=="object"&&typeof window["s_gi"]!="undefined"){var a=s_gi(window.s_adobe_account);var b=window[this.conf.var_name];
...
this.slb(a,c,this.conf.link_name)}this.c_w(this.conf.var_name,"done",0,this.conf.ck_domain,this.conf.ck_path)}else{if(typeof this.conf.count=="undefined")this.conf.count=1;
			if(this.conf.count<=this.conf.check_max){this.conf.count=this.conf.count+1;setTimeout("s_adobe_genesis_dmdbase_i.tl()",this.conf.check_delay)}}};
			...
			d.src=c;document.getElementsByTagName("HEAD")[0].appendChild(d);setTimeout("s_adobe_genesis_dmdbase_i.tl()",this.conf.check_delay)};
	...
	if (s_adobe.pageName) {
		...
		if (s_adobe.pageName.indexOf(":products:")!=-1 && s_adobe.products) {
		  s_adobe.prop2=s_adobe.products.split(";")[1];
	          if (s_adobe.pageName.split(":").length-1 > 2 && typeof(s_adobe.events)!=='undefined') {
		    if (s_adobe.events=='prodView,event3') {
			  s_adobe.events='';
			  s_adobe.products='';
		    }
		  }
...
/**********CAMPAIGN TRACKING DIGITAL MARKETING - MEIDA END********************/
	...
	s_adobe.eVar54=s_adobe.getQueryParam('et_rid');
	s_adobe.eVar55=s_adobe.getQueryParam('et_cid');
...
+"var s=this,n='s_tnt',p=p?p:n,v=v?v:n,r='',pm=false,b=b?b:true;if(s_adobe."
...
if (s_tc_wwwadobecomWCMS.cp({h:["adobe.com"]})) {
if (s_adobe.c_r("omniture_optout")!=1 && s_URL.indexOf("/misc/optout")==-1) {
